...
class WeatherViewModel(
    ...
    init {
        val subscription = publishSubject
            ...
            .switchMap {
                weatherRepository
                    .fetchByCityName(it.city_name, it.days)
                    ...
                    .subscribe(
                        {
                            when(it) {
                                is Resource.Loading -> mainActivityWeatherState.value = MainActivityWeatherState.Loading
                                is Resource.Success -> mainActivityWeatherState.value = MainActivityWeatherState.DataFetched
                                is Resource.Error -> mainActivityWeatherState.value = MainActivityWeatherState.Error("Error happened while fetching data from the server")
                            }
                        },
                        {
                ...
                weatherRepository
                    .getByCityName(it.city_name, it.date, it.days)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .doOnError {
            ...
            .subscribe(
                ...
                {
                    mainActivityWeatherState.value = MainActivityWeatherState.Error("Error happened while fetching data from databse.")
                    Timber.e(it)
                }
            )
